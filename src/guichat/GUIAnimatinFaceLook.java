...
class GUIAnimatinFaceLook {// 顔のオブジェクト
    ...
    public void makeAngryMouth(Graphics g, int mouseSize) {
		int xMiddle = xStart + h / 2;
		int yMiddle = yStart + 3 * w / 4;
		g.drawLine(xMiddle - mouseSize / 2, yMiddle + 5, xMiddle + mouseSize / 9,
				yMiddle - 10);
        g.drawLine(xMiddle - mouseSize / 9, yMiddle - 10, xMiddle + mouseSize / 2,
        yMiddle + 5);
	}
